...
- (void)viewDidLoad {
    ...
    tab.frame = CGRectMake(0, 140, self.view.frame.size.width, self.view.frame.size.height-140);
    tab.dataSource = self;
    tab.delegate = self;
...
-(void)creatView{
    ...
    bigView.frame = CGRectMake(0, 20, self.view.frame.size.width, 120);
...
-(UIView*)creatViewBig{
    ...
    UIView *view = [[UIView alloc]initWithFrame:CGRectMake(0, 140, self.view.frame.size.width, self.view.frame.size.height-140)];
    ...
    for (int i =0; i<6; i++) {
        ...
        CGSize size = [titleArr[i] sizeWithAttributes:[NSDictionary dictionaryWithObjectsAndKeys:[UIFont systemFontOfSize:[UIFONT fontWithDevice:15]],NSFontAttributeName, nil]];
        
        UILabel *labName = [[UILabel alloc]init];
        labName.text = titleArr[i];
        labName.textAlignment = NSTextAlignmentLeft;
        labName.font = [UIFont systemFontOfSize:[UIFONT fontWithDevice:15]];
        ...
        labDetail.font = [UIFont systemFontOfSize:[UIFONT fontWithDevice:13]];
        labDetail.textColor = [UIColor blackColor];
        labDetail.text = [NSString stringWithFormat:@"%@",thingsArr[i]];
        [view addSubview:labDetail];
        
        labDetail.sd_layout
        .leftSpaceToView(view,view.frame.size.width/4+size.width+10)
