...
- (void)setScanView
{
    ...
    readerView.frame = CGRectMake(10, 200, self.view.frame.size.width-20, self.view.frame.size.height-210);
            ...
            if ([[NSFileManager defaultManager] fileExistsAtPath:[NSHomeDirectory() stringByAppendingPathComponent:@"Documents/ids.text"]]) {
                ...
                [HttpRequest getDidComGuestInfo:sym.data guest:idsArr things:itemsArr completeWithData:^(NSMutableArray *guestArr) {
                    ...
                    [UIView animateWithDuration:1 animations:^{
                        tab.frame = CGRectMake(70,SCREENHEIGHT/5, tab.frame.size.width, 250);
                    }];
                }];
            } 
...
-(void)creatViewShow{
    ...
    tab.frame = CGRectMake(70,self.view.frame.size.height , self.view.frame.size.width-140, 0);
    tab.delegate = self;
    tab.dataSource = self;
